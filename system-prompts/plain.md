...
## Self-governed (自律; only on explicit instruction)
 ...
 - A git commit without instruction is allowed only in this mode and only when the repository's CLAUDE.md declares that self-governed commits are permitted. Without that declaration, make the rollback point by stash or copy, not by commit. A push is never made without instruction, in any mode, in any repository. Commit messages state what changed; they carry no conversation history and nothing unrelated to the change.
